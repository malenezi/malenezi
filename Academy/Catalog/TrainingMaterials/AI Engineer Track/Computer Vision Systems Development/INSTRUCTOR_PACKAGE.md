...
| Field | Details |
|---|---|
| **Course Title** | Computer Vision Systems Development |
| **Arabic Title** | تطوير أنظمة الرؤية الحاسوبية |
| **Module Code** | SDA-AIE-212 |
| **Level** | Specialist |
...
| **Tools & Platforms** | PyTorch · torchvision · YOLO/Ultralytics · OpenCV · ONNX |
...
The course is built around a single evolving artefact: **"Tamr Vision"**, a quality-inspection system for a date-packing plant in Al-Qassim. Participants build its input pipeline, fine-tune a grade classifier, train a defect detector and a surface-defect segmenter, curate and re-annotate its dataset, construct a rigorous evaluation harness, and finally export and optimise the models for an edge box mounted over the conveyor. Every lab produces a component of the final capstone, so by Day 4 each participant owns a complete, measured, deployable vision inspection system — the same shape of system they will operate and monitor in SDA-AIE-216 (MLOps).
...
| **Day 4** | Ship it to the line | M7: Deployment & Edge Optimisation · Capstone | 25% | 75% | ONNX-exported, benchmarked edge models + capstone demo of the full inspection system |
...
|---|---|---|---|---|
| 1 | **Why vision systems fail in production** + course kickoff | Recognise the gap between benchmark accuracy and line performance; meet the Tamr Vision scenario and the seven module themes | Interactive lecture + failure-story discussion | 80/20 |
...
- **Golden thread:** every module works on the same Tamr Vision inspection system and the same `dates-qc` dataset. Never introduce a throwaway dataset — always evolve the course artefact. This is what makes the capstone achievable in Day 4 Hours 3–4.
...
- **Prayer and break scheduling:** each "hour" is 50 minutes of instruction + 10 minutes buffer; schedule the long break around Dhuhr. Day 3 Hour 2 (annotation lab) and Day 4 afternoon are deliberately flexible for regional scheduling.
- **Assessment logistics:** rubric scoring happens live during Day 4 Hour 5 demos; collect repository URLs and BENCHMARKS.md at the end of Hour 4 so evaluation-harness results can be verified before demos begin.
...
- **Update mechanism:** models arrive as versioned bundles (ONNX + config + contract version + harness report); staged rollout (shadow mode → one line → fleet); rollback is a config flip, not an SSH session. SDA-AIE-216 turns this paragraph into a course.
...
You are the vision engineer for **"Tamr Vision" (رؤية تمر)**, the quality-inspection system of a date-packing plant in Al-Qassim. Over four days you have built every component: the input pipeline and augmentation policy (M1), the fine-tuned grade classifier (M2), the defect detector with its negotiated operating point (M3), the surface-defect segmenter and area measurement (M4), the curated v2 dataset (M5), the evaluation harness that referees everything (M6), and the exported, quantised, benchmarked edge artefacts (M7). The capstone is the integration plus your own engineering judgement: one system that watches the conveyor, grades every fruit against the plant's rules, and proves its own quality with a harness report. This is the vision-system project named in the course assessment — it absorbs the "vision application project" topic as the course's final integrated build.
...
```
 ...
 v1 ─ v2 (M5)           │        └──── evaluation harness (M6) ────┘       │
   ...
   │                                          │  versioned bundle
   ▼                                          ▼
 audit/AL loop <── decision logs <── EDGE BOX: capture → preprocess (contract)
 (M5 flywheel)                        → detect + segment + classify → grading
                                      rules (8% area, operating points)
...
5. Deployment bundle: ONNX artefacts + config + parity evidence + selected line configuration
6. 5-minute demo: sample-footage run with decisions and logs, one harness insight (your worst slice), one trade-off defended (your selected config), and your remediation plan's top line
7. `DECISIONS.md`: five one-paragraph engineering decisions with rationale (e.g., operating-point choice, 416² acceptance, calibration stratification, fallback behaviour)
...
| M-E: assembled system + self-audit + extension | Day 4 H4 | capstone checklist green |
| M-F: demo + submission | Day 4 H5 | rubric scoring |

## Grading Rubric (100 points)

| Criterion | Weight | 90–100% band | 70–89% band | < 70% band |
|---|---|---|---|---|
| Model quality (classifier, detector, segmenter vs targets) | 20 | All targets met on frozen split; per-class numbers honest and reproducible | One target narrowly missed with credible analysis | Targets missed without analysis; eval on wrong split |
| Data curation & v2 evidence | 15 | Audit triaged correctly, spec fix merged, release complete, delta attributed (clean vs new data) | Release complete but attribution thin | Test split touched, no changelog, or fabricated triage |
| Evaluation harness & analysis | 20 | Deterministic, validated matching, slices declared, taxonomy + costed remediation plan, CI on deltas | Harness works; analysis shallow (no plan or no slices) | Framework numbers copied; no per-slice view |
| Deployment & optimisation | 20 | Parity-tested artefacts, slice-checked INT8, complete trade-off table, budget-justified config | Exported and benchmarked; slice check or justification missing | No parity tests; benchmark without warm-up/percentiles |
| System integration & decision logic | 15 | Full pipeline on footage; grading rules versioned; structured logs with versions and latencies | Runs but logs thin or rules hard-coded | Components never assembled |
| Demo & decisions doc | 10 | Crisp demo; DECISIONS.md shows trade-off literacy across data/eval/deploy | Demo works; rationale thin | Cannot run own system; no decisions doc |
...
- Anti-pattern flags that cap a criterion at 70%: metrics reported at undeployed thresholds; test-split modifications; INT8 adopted with no slice check; parity "tested" on random tensors only; regression deltas claimed without n or CI.
...
3. Multi-object tracking (ByteTrack via Ultralytics) so each physical fruit is counted once across frames — report count accuracy vs the frame-level baseline
...
|---|---|---|
| Lab completion (7 labs) | 35% | checkpoint commits + expected outputs + `BENCHMARKS.md` entries |
| PA-1 + PA-2 | 15% | artefacts + notes |
...
- [ ] Fork/clone course org repos; push checkpoint tags (`lab1-start` … `lab7-start`) and all `sim-*` branches (`sim-interp-skew`, `sim-leaky-split`, `sim-eval-mode`, `sim-class-shift`, `sim-conf-mismatch`, `sim-fat-mask`, `sim-res-320`, `sim-anchor-bias`, `sim-spec-drift`, `sim-threshold-report`, `sim-calib-day`, `sim-wrapper-drift`)
...
- [ ] Dry-run all 7 labs end-to-end on a classroom GPU workstation **and** on the CPU-fallback path (320², reduced epochs, provided checkpoints) — a CV course without a verified GPU plan collapses on Day 2
...
- [ ] `pip install ultralytics opencv-python onnx onnxruntime pandas scikit-learn matplotlib pytest`
...
- Hard rule: capstone assembly (Day 4 Hour 3) starts on time; cut discussion, not build time.
...
- Map each capstone requirement to the module that taught it (one slide: seven requirements, seven modules).
...
*End of instructor package. All code samples target Python 3.12, PyTorch ≥ 2.4, torchvision ≥ 0.19, Ultralytics ≥ 8.3, OpenCV ≥ 4.10, onnxruntime ≥ 1.19. Verify pinned versions in the course lock file before each delivery.*
